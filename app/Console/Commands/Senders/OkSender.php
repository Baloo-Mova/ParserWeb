...
class OkSender extends Command
{
    ...
    public function handle()
    {
        while (true) {
            try {
                $this->content = null;
                $this->account = AccountsData::getSenderAccount(AccountsDataTypes::OK);
                ...
                $sendThis = $this->content->getSendData('ok');

                if (!isset($sendThis) || count($sendThis) < 1 || empty($sendThis['text'])) {
                    $this->account->release();
                ...
                if ($vk->sendMessage($this->content->value, $str_mes)) {
